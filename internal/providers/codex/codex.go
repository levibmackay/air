// Package codex implements the agent.Agent interface for OpenAI Codex CLI,
// driven non-interactively via `codex exec "<prompt>"`.
package codex
...
// Provider implements agent.Agent for Codex CLI.
...
// New returns a Codex provider. Optional model parameter can be specified.
func New(model ...string) agent.Agent {
	...
	return &Provider{
		name:    name,
		model:   m,
		runner:  cliagent.Runner{Binary: "codex"},
